...
namespace PresentationLayer.Pages.Home
{
    public class DetailModel : PageModel
    {
        ...
        // Lightweight JSON endpoint to feed products dropdown in modal (public API style)
        public async Task<IActionResult> OnGetProductsAsync()
        {
            var (success, error, data) = await _productService.SearchAsync(null, null, null, null, null, null);
            if (!success || data == null) return new JsonResult(Array.Empty<object>());
            var result = data.Select(p => new { id = p.Id, name = p.Name, sku = p.Sku }).ToList();
